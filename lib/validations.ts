...
export const createItemSchema = z.object({
  ...
  category: z.enum(["Food", "Beverage", "Others"], {
    message: "Category must be Food, Beverage, or Others",
  }),
...
export const createWaiterSchema = z.object({
  ...
  age: z.coerce
    .number()
    .min(18, "Waiter must be at least 18 years old")
    .max(70, "Invalid age"),
  experience: z.coerce
    .number()
...
export const createTableSchema = z.object({
  tableNo: z.coerce
    .number()
    .min(0, "Table number must be at least 1")
    .max(999, "Invalid table number"),
  capacity: z.coerce
    .number()
...
export const pityCashSchema = z
  .object({
    amount: z.number().min(0, "Enter valid Amount Here"),
...
export const pityCashSchemaUpdate = pityCashSchema.extend({
  id: z.number().min(1, "Pity Cash ID is required"),
});

export const creditLevelSchema = z.object({
  level: z.enum(["Bronze", "Silver", "Gold"], {
    message: "Credit level must be Bronze, Silver, or Gold",
  }),
  requiredAmount: z.number().min(0, "Amount must be positive"),
...
export const creditRegistrationSchemaUpdate = creditRegistrationSchema.extend({
  id: z.number().min(1, "Credit Registration ID is required"),
});
...
export const ItemRegistrationSchemaUpdate = ItemRegistrationSchema.extend({
  id: z.number().min(1, "Item Registration id is required")
})
...
export const credentialFormSchema = z
  ...
  .refine(
    (data) => data.passwordInfo.Password === data.passwordInfo.confirmPassword,
    {
      message: "Passwords don't match",

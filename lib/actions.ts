...
async function sequentialAuthorizePurchaseRequestsManager(
  unique: number[],
): Promise<{ id: number; status: string }[]> {
  const ok: { id: number; status: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await authorizePurchaseRequestManagerApi(id, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Authorized ${ok.length} request(s)`);
  if (failed.length) {
    toast.error(`Some authorizations failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
  }
  return ok;
...
async function sequentialRejectPurchaseRequestsManager(
  unique: number[],
  reason: string,
): Promise<PurchaseRequestRow[]> {
  const ok: PurchaseRequestRow[] = [];
  ...
  for (const id of unique) {
    try {
      ok.push(
        await rejectPurchaseRequestManagerApi(id, reason, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
...
async function sequentialCheckItemRegistrationsCC(
  ...
  const ok: { id: number; approvalStatus: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await checkItemRegistrationCCApi(id, costControllerProfileId, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Checked ${ok.length} registration(s)`);
  if (failed.length) {
    toast.error(`Some checks failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
  }
  return ok;
...
async function sequentialApproveItemRegistrationsFinance(
  unique: number[],
): Promise<{ id: number; approvalStatus: string }[]> {
  const ok: { id: number; approvalStatus: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await approveItemRegistrationFinanceApi(id, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Approved ${ok.length} registration(s)`);
  if (failed.length) {
    toast.error(`Some approvals failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
...
async function sequentialRejectItemRegistrationsFinance(
  unique: number[],
  reason: string,
): Promise<{ id: number; approvalStatus: string }[]> {
  const ok: { id: number; approvalStatus: string }[] = [];
  ...
  for (const id of unique) {
    try {
      ok.push(
        await rejectItemRegistrationFinanceApi(id, reason, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Rejected ${ok.length} registration(s)`);
  if (failed.length) {
    toast.error(`Some rejections failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
...
async function sequentialAuthorizeItemRegistrationsManager(
  unique: number[],
): Promise<{ id: number; approvalStatus: string }[]> {
  const ok: { id: number; approvalStatus: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await authorizeItemRegistrationManagerApi(id, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Authorized ${ok.length} registration(s)`);
  if (failed.length) {
    toast.error(`Some authorizations failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
...
async function sequentialApproveStockOutRequestsFinance(
  unique: number[],
): Promise<{ id: number; status: string }[]> {
  const ok: { id: number; status: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await approveStockOutRequestFinanceApi(id, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Approved ${ok.length} movement(s)`);
  if (failed.length) {
    toast.error(`Some approvals failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
...
async function sequentialAuthorizeStockOutRequestsManager(
  unique: number[],
): Promise<{ id: number; status: string }[]> {
  const ok: { id: number; status: string }[] = [];
  const failed: string[] = [];
  for (const id of unique) {
    try {
      ok.push(
        await authorizeStockOutRequestManagerApi(id, {
          suppressSuccessToast: true,
        }),
      );
    } catch (err: unknown) {
  ...
  if (ok.length) toast.success(`Authorized ${ok.length} movement(s)`);
  if (failed.length) {
    toast.error(`Some authorizations failed (${failed.length})`, {
      description: failed.slice(0, 5).join(" · "),
    });
...
export async function authorizePurchaseRequestsManagerBatchApi(
  ids: number[],
): Promise<{ id: number; status: string }[]> {
  const unique = [...new Set(ids)].filter((id) => id > 0);
  if (unique.length === 0) return [];
  if (unique.length === 1) {
    const one = await authorizePurchaseRequestManagerApi(unique[0]);
    return [{ id: one.id, status: one.status }];
  }
  return sequentialAuthorizePurchaseRequestsManager(unique);
}

export async function rejectPurchaseRequestsManagerBatchApi(
  ids: number[],
  ...
  if (unique.length === 0) return [];
  if (unique.length === 1) {
    const one = await rejectPurchaseRequestManagerApi(unique[0], reason);
    return [one];
  }
  return sequentialRejectPurchaseRequestsManager(unique, reason);
}
...
export async function approveStockOutRequestsFinanceBatchApi(
  ids: number[],
): Promise<{ id: number; status: string }[]> {
  const unique = [...new Set(ids)].filter((id) => id > 0);
  if (unique.length === 0) return [];
  if (unique.length === 1) {
    const one = await approveStockOutRequestFinanceApi(unique[0]);
    return [{ id: one.id, status: one.status }];
  }
  return sequentialApproveStockOutRequestsFinance(unique);
}

export async function authorizeStockOutRequestsManagerBatchApi(
  ids: number[],
): Promise<{ id: number; status: string }[]> {
  const unique = [...new Set(ids)].filter((id) => id > 0);
  if (unique.length === 0) return [];
  if (unique.length === 1) {
    const one = await authorizeStockOutRequestManagerApi(unique[0]);
    return [{ id: one.id, status: one.status }];
  }
  return sequentialAuthorizeStockOutRequestsManager(unique);
}
...
export async function approveItemRegistrationsFinanceBatchApi(
  ...
  return sequentialApproveItemRegistrationsFinance(unique);
}

export async function rejectItemRegistrationsFinanceBatchApi(
  ids: number[],
  reason = "Rejected by finance",
): Promise<{ id: number; approvalStatus: string }[]> {
  const unique = [...new Set(ids)].filter((id) => id > 0);
  if (unique.length === 0) return [];
  if (unique.length === 1) {
    const one = await rejectItemRegistrationFinanceApi(unique[0], reason);
    return [{ id: one.id, approvalStatus: one.approvalStatus }];
  }
  return sequentialRejectItemRegistrationsFinance(unique, reason);
}
...
export async function authorizeItemRegistrationsManagerBatchApi(
  ...
  return sequentialAuthorizeItemRegistrationsManager(unique);
}

"use client";

import { useCallback, useState } from "react";
import { toast } from "sonner";
import { useConcurrentActions } from "@/hooks/useConcurrentActions";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { PendingButton } from "@/components/ui/pending-button";
import {
  CreateItemStatus,
  UpdateItemRegistration,
  DeleteItemRegistration,
  type ItemRegistration,
} from "@/lib/actions";
import { isVatEnabled } from "@/lib/hotelInventoryPayment";
import { HOTEL_STORE_STOCK_OUT_STAKEHOLDERS } from "@/lib/hotelDailyStation";
import { HotelDayPicker } from "@/components/hotel/HotelDayPicker";
import { toYmdLocal, ymdToRegistrationTimestamp } from "@/lib/hotelDateYmd";
import type { DataTableRef } from "@/app/StoreItems/data-table";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Label } from "@/components/ui/label";
import { Badge } from "@/components/ui/badge";
import { Send } from "lucide-react";

type MovementKind = "STOCK_OUT" | "WASTAGE" | "RETURN_SUPPLIER";

/** ItemStatus.status label written for each movement type (matches single-row path). */
const STATUS_LABEL: Record<MovementKind, string> = {
  STOCK_OUT: "Stock Out",
  WASTAGE: "Wastage",
  RETURN_SUPPLIER: "Returned to Supplier",
};

type LineDraft = {
  registrationId: number;
  itemName: string;
  onHand: number;
  measuredBy: string;
  movement: MovementKind;
  amount: string;
  /** Stock-out: predefined stakeholder code (from HOTEL_STORE_STOCK_OUT_STAKEHOLDERS) */
  stakeholder: string;
  /** Stock-out: optional free-text destination overrides the stakeholder */
  customStation: string;
  /** Wastage / return reason */
  reason: string;
};

function defaultAmountForRow(row: ItemRegistration): string {
  const onHand = Math.max(0, Number(row.amount) || 0);
  const def = Math.min(1, onHand);
  return def > 0 ? String(def) : "0";
}

function rowsToDrafts(selected: ItemRegistration[]): LineDraft[] {
  return selected.map((row) => ({
    registrationId: row.id,
    itemName: row.name,
    onHand: Number(row.amount) || 0,
    measuredBy: row.measuredBy || "Piece",
    movement: "STOCK_OUT",
    amount: defaultAmountForRow(row),
    stakeholder: "",
    customStation: "",
    reason: "",
  }));
}

function stockOutDestination(line: LineDraft): string {
  const custom = line.customStation.trim();
  if (custom) return custom;
  return line.stakeholder.trim();
}

/**
 * Café store batch stock movements. Unlike the hotel bar (which opens an
 * approval pipeline via stock-out requests), café movements are applied
 * immediately: each line writes an ItemStatus row, decrements the registration
 * quantity, and removes the registration when it reaches zero.
 */
export function CafeInventoryBatchMovementBar({
  selected,
  tableRef,
  refresh,
}: {
  selected: ItemRegistration[];
  tableRef: React.RefObject<DataTableRef | null>;
  refresh?: () => void;
}) {
  const [open, setOpen] = useState(false);
  const [lines, setLines] = useState<LineDraft[]>([]);
  const [defaultStakeholder, setDefaultStakeholder] = useState("");
  const [movementDateYmd, setMovementDateYmd] = useState(() =>
    toYmdLocal(new Date()),
  );
  const { isPending, run } = useConcurrentActions();
  const batchKey = "cafe-inventory-batch-movements";

  const updateLine = useCallback((id: number, patch: Partial<LineDraft>) => {
    setLines((prev) =>
      prev.map((l) => (l.registrationId === id ? { ...l, ...patch } : l)),
    );
  }, []);

  const applyDefaultStakeholderToAll = useCallback((value: string) => {
    setDefaultStakeholder(value);
    setLines((prev) =>
      prev.map((l) =>
        l.movement === "STOCK_OUT" ? { ...l, stakeholder: value } : l,
      ),
    );
  }, []);

  const handleSubmit = () => {
    if (!movementDateYmd.trim()) {
      toast.error("Select the movement date");
      return;
    }
    const actionDate = ymdToRegistrationTimestamp(movementDateYmd);
    void run(batchKey, async () => {
      const rowById = new Map(selected.map((r) => [r.id, r]));

      // Validate every line up front so we don't apply a partial batch.
      for (const line of lines) {
        const row = rowById.get(line.registrationId);
        if (!row) continue;
        const q = Number(line.amount);
        if (!Number.isFinite(q) || q <= 0) {
          toast.error(`Enter a valid quantity for “${line.itemName}”.`);
          return;
        }
        if (q > row.amount) {
          toast.error(
            `“${line.itemName}”: quantity cannot exceed ${row.amount} on hand.`,
          );
          return;
        }
        if (line.movement === "STOCK_OUT") {
          if (!stockOutDestination(line)) {
            toast.error(
              `Select or enter a stakeholder for “${line.itemName}”, or use “Apply stakeholder to all” above.`,
            );
            return;
          }
        } else if (!line.reason.trim()) {
          toast.error(
            `Enter a reason for “${line.itemName}” (wastage / return).`,
          );
          return;
        }
      }

      let ok = 0;
      let failed = 0;

      for (const line of lines) {
        const row = rowById.get(line.registrationId);
        if (!row) {
          failed++;
          continue;
        }
        const q = Number(line.amount);
        const statusBy =
          line.movement === "STOCK_OUT"
            ? stockOutDestination(line)
            : line.reason.trim();
        try {
          await CreateItemStatus({
            name: row.name,
            imageUrl: row.imageUrl,
            category: row.category,
            amount: q,
            measuredBy: row.measuredBy,
            unitPrice: row.unitPrice,
            actionDate,
            supplierName: row.supplierName,
            supplierPhone: row.supplierPhone,
            Address: row.Address,
            purchaseWithVat: isVatEnabled(row.purchaseWithVat),
            supplierTinNumber: (row.supplierTinNumber ?? "").trim(),
            paidAmount: row.paidAmount,
            status: STATUS_LABEL[line.movement],
            statusBy,
            HotelName: row.HotelName,
          });
          const nextAmount = Number(row.amount) - q;
          await UpdateItemRegistration({ ...row, amount: nextAmount });
          if (nextAmount === 0) {
            await DeleteItemRegistration(row.id);
          }
          ok++;
        } catch (e: unknown) {
          failed++;
          const msg =
            e instanceof Error
              ? e.message
              : `Failed to record movement for ${line.itemName}`;
          toast.error(msg);
        }
      }

      if (ok > 0) {
        toast.success(
          `Recorded ${ok} movement${ok === 1 ? "" : "s"}${
            failed ? ` (${failed} failed)` : ""
          }`,
        );
      } else if (failed > 0) {
        toast.error("No movements were recorded.");
      }
      tableRef.current?.resetRowSelection();
      setOpen(false);
      if (ok > 0) refresh?.();
    });
  };

  if (selected.length === 0) return null;

  return (
    <>
      <div className="border-b border-border/60 bg-muted/25 px-4 py-4 sm:px-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          <p className="text-sm text-muted-foreground min-w-0">
            <span className="font-semibold tabular-nums text-foreground">
              {selected.length}
            </span>{" "}
            line{selected.length === 1 ? "" : "s"} selected. Open the editor to
            set movement, quantity, and stakeholder per item.
          </p>
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <Button
              type="button"
              className="gap-2 font-semibold shadow-sm"
              onClick={() => {
                setLines(rowsToDrafts(selected));
                setDefaultStakeholder("");
                setMovementDateYmd(toYmdLocal(new Date()));
                setOpen(true);
              }}
            >
              <Send className="size-4 shrink-0" aria-hidden />
              Apply movements
            </Button>
            <Button
              type="button"
              size="sm"
              variant="outline"
              className="text-muted-foreground"
              onClick={() => tableRef.current?.resetRowSelection()}
            >
              Clear selection
            </Button>
          </div>
        </div>
      </div>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          showCloseButton
          className="flex max-h-[min(92vh,720px)] w-full max-w-xl flex-col gap-0 overflow-hidden p-0 sm:max-w-xl"
        >
          <div className="shrink-0 border-b border-border/60 bg-linear-to-b from-muted/40 to-muted/10 px-5 pt-5 pb-4 pr-12">
            <DialogHeader className="space-y-2 text-left">
              <DialogTitle className="text-base font-semibold tracking-tight sm:text-lg">
                Batch stock movements
              </DialogTitle>
              <DialogDescription className="text-sm text-pretty leading-relaxed">
                Each card is one movement applied immediately. Choose movement
                and quantity, then a stakeholder (stock-out) or a reason (wastage
                / return).
              </DialogDescription>
            </DialogHeader>
          </div>

          <ScrollArea className="min-h-0 flex-1 overflow-x-hidden">
            <div className="space-y-3 px-4 py-4 sm:px-5">
              <HotelDayPicker
                id="cafe-batch-movement-date"
                label="Movement date"
                value={movementDateYmd}
                onChange={setMovementDateYmd}
                compact
              />
              {lines.some((l) => l.movement === "STOCK_OUT") ? (
                <div className="space-y-1.5 rounded-lg border border-dashed border-border/70 bg-muted/20 p-3">
                  <Label htmlFor="cafe-default-stakeholder">
                    Apply stakeholder to all stock-out lines
                  </Label>
                  <Select
                    value={defaultStakeholder || undefined}
                    onValueChange={applyDefaultStakeholderToAll}
                  >
                    <SelectTrigger
                      id="cafe-default-stakeholder"
                      className="h-10 w-full"
                    >
                      <SelectValue placeholder="Select once — applies to every stock-out line" />
                    </SelectTrigger>
                    <SelectContent>
                      {HOTEL_STORE_STOCK_OUT_STAKEHOLDERS.map((s) => (
                        <SelectItem key={s} value={s}>
                          {s}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <p className="text-xs text-muted-foreground">
                    Use this when sending many items to the same station. You can
                    still override any line below.
                  </p>
                </div>
              ) : null}
              {lines.map((line) => (
                <div
                  key={line.registrationId}
                  className="rounded-xl border border-border/70 bg-card/80 p-4 shadow-sm ring-1 ring-black/4 dark:ring-white/6"
                >
                  <div className="mb-3 flex flex-wrap items-start justify-between gap-2 gap-y-1">
                    <p className="min-w-0 flex-1 text-sm font-medium leading-snug text-foreground">
                      {line.itemName}
                    </p>
                    <Badge
                      variant="secondary"
                      className="shrink-0 tabular-nums font-normal"
                    >
                      {line.onHand} {line.measuredBy}
                    </Badge>
                  </div>

                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5 sm:col-span-1">
                      <Label htmlFor={`cafe-mov-${line.registrationId}`}>
                        Movement
                      </Label>
                      <Select
                        value={line.movement}
                        onValueChange={(v) =>
                          updateLine(line.registrationId, {
                            movement: v as MovementKind,
                          })
                        }
                      >
                        <SelectTrigger
                          id={`cafe-mov-${line.registrationId}`}
                          className="h-10 w-full"
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="STOCK_OUT">Stock out</SelectItem>
                          <SelectItem value="WASTAGE">Wastage</SelectItem>
                          <SelectItem value="RETURN_SUPPLIER">
                            Return to supplier
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1.5 sm:col-span-1">
                      <Label htmlFor={`cafe-qty-${line.registrationId}`}>
                        Quantity
                      </Label>
                      <Input
                        id={`cafe-qty-${line.registrationId}`}
                        type="number"
                        min={0.01}
                        step="any"
                        className="h-10 tabular-nums"
                        value={line.amount}
                        onChange={(e) =>
                          updateLine(line.registrationId, {
                            amount: e.target.value,
                          })
                        }
                      />
                    </div>

                    {line.movement === "STOCK_OUT" ? (
                      <div className="space-y-2 sm:col-span-2">
                        <Label>Stakeholder</Label>
                        <Select
                          value={line.stakeholder || undefined}
                          onValueChange={(v) =>
                            updateLine(line.registrationId, { stakeholder: v })
                          }
                        >
                          <SelectTrigger className="h-10 w-full">
                            <SelectValue placeholder="Select stakeholder" />
                          </SelectTrigger>
                          <SelectContent>
                            {HOTEL_STORE_STOCK_OUT_STAKEHOLDERS.map((s) => (
                              <SelectItem key={s} value={s}>
                                {s}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        <Input
                          className="h-10 text-sm"
                          placeholder="Optional: custom destination"
                          value={line.customStation}
                          onChange={(e) =>
                            updateLine(line.registrationId, {
                              customStation: e.target.value,
                            })
                          }
                        />
                      </div>
                    ) : (
                      <div className="space-y-1.5 sm:col-span-2">
                        <Label htmlFor={`cafe-reason-${line.registrationId}`}>
                          Reason (required)
                        </Label>
                        <Input
                          id={`cafe-reason-${line.registrationId}`}
                          className="h-10"
                          placeholder="e.g. spoilage, wrong delivery…"
                          value={line.reason}
                          onChange={(e) =>
                            updateLine(line.registrationId, {
                              reason: e.target.value,
                            })
                          }
                        />
                      </div>
                    )}
                  </div>
                </div>
              ))}
            </div>
          </ScrollArea>

          <DialogFooter className="shrink-0 flex-col gap-3 border-t border-border/60 bg-muted/20 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <p className="text-xs text-muted-foreground order-2 sm:order-1 sm:max-w-[55%]">
              Stock-out needs a stakeholder or custom destination. Wastage and
              returns need a reason. Movements apply immediately.
            </p>
            <div className="flex w-full flex-col-reverse gap-2 order-1 sm:order-2 sm:w-auto sm:flex-row sm:justify-end">
              <Button
                type="button"
                variant="outline"
                onClick={() => setOpen(false)}
              >
                Cancel
              </Button>
              <PendingButton
                pending={isPending(batchKey)}
                onClick={() => handleSubmit()}
                className="gap-2 min-w-40 font-semibold"
              >
                <Send className="size-4 shrink-0" aria-hidden />
                Apply {lines.length} movement{lines.length === 1 ? "" : "s"}
              </PendingButton>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  );
}

...
function readStored(): StoredState {
  try {
    ...
    return { playlistId, volume, muted: Boolean(parsed.muted) };
  } catch {
    return {
      playlistId: CLASSICAL_PLAYLISTS[0].id,
      volume: CLASSICAL_DEFAULT_VOLUME,
...
export function ClassicalAmbientPlayer() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      CLASSICAL_STORAGE_KEY,
      JSON.stringify({ playlistId, volume, muted }),
    );
  }, [mounted, playlistId, volume, muted]);
  ...
  return (
    <div
      ...
      onMouseEnter={computer ? openPanel : undefined}
      onMouseLeave={computer ? scheduleClose : undefined}
    >
      <audio
        ref={audioRef}
        preload="none"

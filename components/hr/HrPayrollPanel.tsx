  ...
  const columns = useMemo<ColumnDef<HrPayslip>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Net",
        cell: ({ row }) => (
          <span className="font-semibold">{formatETB(row.original.netPayETB)}</span>
        ),
      },
      {
        id: "actions",
  ...
  return (
    <HrPanelShell>
      <HrSectionCard
        ...
        description="A period covers the calendar month. Closing generates payslips from current salaries."
        ...
        actions={
          selected ? <HrStatusBadge status={selected.status} /> : null
        }
      >
        <div className="flex flex-wrap gap-3">
          ...
          {selected && selected.status !== "closed" ? (
            <HrConfirmAction
              ...
              trigger={<Button variant="secondary">Close & generate payslips</Button>}
              onConfirm={async () => {
                try {
      ...
      >
        {payslips.length ? (
          <DataTable
            ...
            footerSummary={(rows) => (
              <span className="font-medium">
                Net {formatETB(rows.reduce((sum, row) => sum + (row.netPayETB || 0), 0))}
              </span>
            )}
          />
        ) : (
          <HrEmptyState
            ...
            description="Create this month’s period and close it after salaries are current."
          />
        )}
      </HrSectionCard>
      ...
      <Dialog open={Boolean(edit)} onOpenChange={(open) => !open && setEdit(null)}>
        <DialogContent className={responsiveFormDialogClassName}>
          <DialogHeader>
            ...
            <DialogDescription>
              Tips, overtime, and deductions for {edit?.employee?.fullName || "this employee"}.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                value={draft.tipsETB}
                onChange={(e) =>
                  setDraft((d) => ({ ...d, tipsETB: Number(e.target.value) || 0 }))
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDraft((d) => ({ ...d, overtimeETB: Number(e.target.value) || 0 }))
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              onClick={async () => {
                ...
                try {
                  await upsertHrPayslipApi({
                    ...
                    overtimeETB: parsed.data.overtimeETB ?? edit.overtimeETB,
                    tipsETB: parsed.data.tipsETB,
                    deductionsETB: parsed.data.deductionsETB ?? edit.deductionsETB,
                  });
